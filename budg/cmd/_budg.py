...
from budg.utils.importer import ImportFromStringError, import_from_string
...
def config_from_file(file: str | None = None, format: str | None = None) -> Mapping[str, Any]:
    ...
    try:
        ...
        raise click.ClickException(f"{file!r}: {decoder.name}: {exc}")
    except OSError as exc:
        raise click.ClickException(f"{file!r}: {exc.strerror}")
...
def config_from_import(import_str: str) -> Mapping[str, Any]:
    ...
    try:
        ...
        raise click.ClickException(excmsg.format(exc))
    # `obj.__name__` not guaranteed to exist
    name = import_str.partition(":")[2].rsplit(".", 1)[-1]
    try:
        ...
        msg = excmsg.format("Object '{name}' must be of type '(tuple[int, int, int]) -> Mapping[str, Any]'")
        raise click.ClickException(msg.format(name=name))
    # `dataclass_from_dict` shouldn't complain about non-str keys
    ret: Any | Mapping[str, Any] = obj(VERSION_INFO)
